...
// 	m_bodyMesh[0]->setTexture("../Models/brick1.jpg",shaderDefault->GetProgramID());
	...
	do{
		...
		GLuint cameraPos = glGetUniformLocation(shaderDefault->GetProgramID(), "WorldCameraPosition");
		glUniform3f(cameraPos, m_physicsLabCamera->position.x,m_physicsLabCamera->position.y,m_physicsLabCamera->position.z);

		GLuint dsb = glGetUniformLocation(shaderDefault->GetProgramID(), "DrawSkyBox");
		...
		GLuint dsbs = glGetUniformLocation(shaderDefault->GetProgramID(), "DrawSkyBox");
		glUniform1i(dsbs, false);

		GLuint mc = glGetUniformLocation(shaderDefault->GetProgramID(), "MaterialColor");
		glUniform4f(mc, 0.5f, 0.5f, 0.5f, 1.0f);

		GLuint rf = glGetUniformLocation(shaderDefault->GetProgramID(), "ReflectFactor");
...
{
	createShaders(shaderDefault, "../Shader/cubemap_reflect.vs", "../Shader/cubemap_reflect.ps");
}
